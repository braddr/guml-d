...
version(none)
{
    int i;
    char[8] n;
    char[256] filename; // TODO: why does this code copy rather than rely on create_string to do it
    ...
    insert_hash(create_string("FILENAME"), create_string(filename.ptr), calc_hash("FILENAME"), HASH_ENV);
    insert_hash(create_string("BASE_DIR"), create_string(GUMLROOT), calc_hash("BASE_DIR"), HASH_ENV);

    if (argc > 2)
    {
        for (i = 2; i < argc; i++)
        {
            size_t len = sprintf (n, "ARG_%d", i - 1);
            insert_hash(create_string(n[0 .. len]), create_string(argv[i]), calc_hash(n[0 .. len]), HASH_ARG);
        }
    }
}
}
...
version(ARG_HANDLE_USE_ONLY_GET_FORMAT)
{
    ...
    if (env_ptr)
    {
        insert_hash(create_string("QUERY_STRING"), create_string(env_ptr), calc_hash("QUERY_STRING"), HASH_ENV);
        str = strdup (env_ptr);
        p = strtok (str, "&");
        while (p)
        { 
            ...
            { 
                *q++ = 0;
                char * hdp = http_decode(p);
                size_t len = strlen(hdp);
                insert_hash(create_string(hdp, len), create_string(http_decode(q)), calc_hash(hdp[0 .. len]), HASH_FORM);
            }
            p = strtok (null, "&");
        }
        free(str);
...
else version(ARG_HANDLE_BOTH_FORMATS)
{
    ...
    if (env_ptr)
    {
        ...
        while (p && i < 10000)
        {
            size_t len = sprintf (n, "ARG_%d", i);
            insert_hash(create_string(n[0 .. len]), create_string(p), calc_hash(n[0 .. len]), HASH_ARG);
            p = strtok (null, " ");
            i++;
        }
        free (str);
        ...
        while (p)
        {
            ...
            if (q)
            {
                *q++ = 0;
                char *hdp = http_decode(p);
                char[] hdpa = hdp[0 .. strlen(hdp)];
                insert_hash(create_string(hdpa), create_string(http_decode(q)), calc_hash(hdpa), HASH_FORM);
            }
            p = strtok (null, "&");
        }
        free(str2);
...
else
{
    ...
    if (env_ptr)
    {
        ...
        while (p && i < 10000)
        {
            size_t len = sprintf (n.ptr, "ARG_%d", i);
            insert_hash(create_string(n[0 .. len]), create_string(p), calc_hash(n[0 .. len]), HASH_ARG);
            p = strtok (null, " ");
            i++;
        }
...
version (FASTCGI)
            ...
            insert_hash(create_string("HTTP_BODY"), create_string(data), calc_hash("HTTP_BODY"), HASH_FORM);
            ...
            while (p)
            {
                ...
                {
                    *q++ = 0;
                    insert_hash(create_string(p, q-p-1), create_string(http_decode(q)), calc_hash(p[0 .. q-p-1]), HASH_FORM);
                }
                p = strtok (null, "&");
            }
            free (data);
...
void setup_cookie_args ()
{
    ...
    if (env_str)
    {
        ...
        while (p)
        {
            ...
            {
                *q++ = 0;
                insert_hash(create_string(p, q-p-1), create_string(q), calc_hash(p[0 .. q-p-1]), HASH_COOKIE);
            }
            p = strtok (null, ";");
            if (p)
                p++;
...
void setup_extract_parts(char *pi, char *pt)
{
    ...
    insert_hash(create_string("BASE_DIR"), data, calc_hash("BASE_DIR"), HASH_ENV);

    /* want to end of string, so no need to make our own temp string */
    insert_hash(create_string("FILENAME"), create_string(pt_ptr), calc_hash("FILENAME"), HASH_ENV);

    /* PATH will be the data between pt_ptr and pt_last_lash, inclusive */
    data = cast(Data*)calloc(1, Data.sizeof);
    add_string(data, pt_ptr, pt_last_slash-pt_ptr+1);
    insert_hash(create_string("PATH"), data, calc_hash("PATH"), HASH_ENV);
}
...
void setup_environment (string[] args)
{
    ...
    if (tmp)
        insert_hash(create_string("REQUEST_METHOD"), create_string(tmp), calc_hash("REQUEST_METHOD"), HASH_ENV);

    iscomm = (tmp == null);
    if (iscomm)
        insert_hash(create_string("USER"), create_string("manual"), calc_hash("USER"), HASH_ENV);
    else
    {
        tmp = GETENV ("REMOTE_USER");
        if (tmp)
            insert_hash(create_string("USER"), create_string(tmp), calc_hash("USER"), HASH_ENV);
    }
    ...
    else
    {
        ...
        if (tmp)
        {
            ...
            servername[240] = 0;
            insert_hash(create_string("SERVERDOMAIN"), create_string(servername.ptr), calc_hash("SERVERDOMAIN"), HASH_ENV);
        }
    ...
    insert_hash(create_string("SERVERNAME"), create_string(servername.ptr), calc_hash("SERVERNAME"), HASH_ENV);
    ...
    if (tmp)
        insert_hash(create_string("SCRIPT_NAME"), create_string(tmp), calc_hash("SCRIPT_NAME"), HASH_ENV);

    tmp = GETENV ("HTTP_REFERER");
    if (tmp)
        insert_hash(create_string("HTTP_REFERER"), create_string(tmp), calc_hash("HTTP_REFERER"), HASH_ENV);

    tmp = GETENV ("HTTP_USER_AGENT");
    if (tmp)
        insert_hash(create_string("HTTP_USER_AGENT"), create_string(tmp), calc_hash("HTTP_USER_AGENT"), HASH_ENV);

    tmp = GETENV ("REMOTE_HOST");
    if (tmp)
        insert_hash(create_string("REMOTE_HOST"), create_string(tmp), calc_hash("REMOTE_HOST"), HASH_ENV);
    else
        insert_hash(create_string("REMOTE_HOST"), create_string("unknown"), calc_hash("REMOTE_HOST"), HASH_ENV);

    tmp = GETENV("REMOTE_ADDR");
    if (tmp)
        insert_hash(create_string("REMOTE_ADDR"), create_string(tmp), calc_hash("REMOTE_ADDR"), HASH_ENV);
    else
        insert_hash(create_string("REMOTE_ADDR"), create_string("unknown"), calc_hash("REMOTE_ADDR"), HASH_ENV);
}
